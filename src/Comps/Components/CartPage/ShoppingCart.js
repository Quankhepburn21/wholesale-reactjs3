...
function ShoppingCart() {// state nằm ở redux toolkit đã được xử lý
  ...
  console.log("LIST DAA", data)

  // Giảm số lượng item
  const decrementHandler = (item) => {
    // So sánh về object id
    const itemSelect = data.find((itemCart) => JSON.stringify(itemCart._id) === JSON.stringify(item._id))
    if (itemSelect.quantity - 1 > 0) {
      dispatch(cartActions.updateCart({ id: item._id, quantity: -1 }))
    } else {
      dispatch(cartActions.removeCart(item._id))
    }
        ...
        <tbody className={classes["list-item"]}>
          {data.map((item, index) => (
            <tr key={item._id.$oid} className="mb-3 align-middle">
              ...
              <td>
                <span onClick={() => decrementHandler(item)}>
                  <svg
                ...
                {item.quantity}
                <span onClick={() => incrementHandler(item)}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#333333"
                    viewBox="0 0 256 256"
